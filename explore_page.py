...
def show_explore_page():
    st.title("Explore IT Engineer Salaries In Indonesia")
    ...
    st.write("""#### Number of Data from different province""")

    st.pyplot(fig1)
    
    st.write(
        """
    #### Mean Salary Based On Province
    """
    )

    data = df.groupby(["Province"])["Salary"].mean().sort_values(ascending=True)
    st.bar_chart(data)
